...
using TagList = std::list<grpc_demo::TagInfo>;
...
static void processRpcs(std::mutex *incoming_tags_mutex,
  ...
  while (true) {
    ...
    while (!tags.empty()) {
      grpc_demo::TagInfo tagInfo = tags.front();
      tags.pop_front();
...
int main(int argc, char **argv) {
  ...
  std::string db_content = grpc_demo::util::GetDbFileContent(argc, argv);
  grpc_demo::ServerImpl server(db_content, incoming_tags_mutex, incoming_tags);
  server.Run();
